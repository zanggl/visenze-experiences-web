    ...
    <div className='relative flex w-full items-center justify-center bg-primary py-4'>
      {
        ...
        ? (
          <>
            <Button isIconOnly className='absolute left-5 top-3 bg-transparent' onClick={onBackHandler}>
              <BackIcon className='size-6'/>
            </Button>
            <p className='hidden text-2xl text-primary md:block'>{resultScreenTitle}</p>
          </>
        )
        : (
          <p className='widget-title text-center text-primary'>
      ...
      <Button isIconOnly className='absolute right-5 top-3 bg-transparent' onClick={onCloseHandler}>
