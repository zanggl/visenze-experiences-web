...
const render = (client: WidgetClient, fieldMappings: Record<any, any>, configs: WidgetConfig): void => {
...
const initWidget = (
  ...
  const result = init(initConfig, fieldMappings, WidgetType.EMBEDDED_SEARCH_RESULTS, '/');
  if (!result) {
    return undefined;
  }
...
(function (context: Window): void {
  const widgetType = `wigmix_${WidgetType.EMBEDDED_SEARCH_RESULTS}`;
